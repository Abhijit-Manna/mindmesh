...
# ==============================================================================
    ...
    html_content = res.get("result") or "No HTML output returned."
    ...
    with col_btn1:
        st.caption(f"Status: {res.get('status', 'completed')} • Saved: `{res.get('file_saved', 'outputs/' + run_id + '.html')}`")
    with col_btn2:
        st.download_button(
            label="📥 Download HTML (.html)",
            data=html_content,
            file_name=f"blueprint-{run_id}.html",
            mime="text/html",
